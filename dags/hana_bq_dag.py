...
CONFIG_GCS_DIR = (
    gsasia-northeast3-hana-bq-com-2c023c7f-bucket
    dagshana_bqconfigs
)

SEOUL = pendulum.timezone(AsiaSeoul)
...
def load_enabled_runs() - list[
    ...
    if not CONFIG_DIR_PATH.exists()
        raise FileNotFoundError(
            fConfig directory not found {CONFIG_DIR_PATH}
        )
    ...
    if not config_files
        raise ValueError(
            fNo YAML files found {CONFIG_DIR_PATH}
        )
    ...
    for config_path in config_files
        ...
        if config.get(version) != 1
            raise ValueError(
                fUnsupported version {config_path.name}
            )
        ...
        if config_path.stem.upper() != table_id.upper()
            raise ValueError(
                fFilename and table id are different 
...
def create_loader_dag(
    ...
    dag_id = (
        fhana_bq_{normalize_name(table_id)}_
    ...
    with DAG(
        ...
        CloudRunExecuteJobOperator(
            task_id=(
                fload_{normalize_name(table_id)}_
                f{normalize_name(run_name)}
            ),
